...
// ---- Scaffold: CLI Tool ----
function cmdCLI() {
  const projectName = process.argv[3] || 'my-cli-tool';
  ...
  fs.writeFileSync(path.join(projectName, 'package.json'), JSON.stringify({
    ...
    scripts: { start: 'node cli.js', dev: 'quill run cli.quill', build: 'quill build cli.quill --standalone' }
  }, null, 2) + '\n');

  fs.writeFileSync(path.join(projectName, 'cli.quill'), `-- CLI Tool built with Quill
...
if name is nothing:
  ...
  say ""
  say "Usage: " + colors.bold("${projectName} <name> [--verbose]")
  exitWith(1)
...
`);
  ...
  console.log(`\n${GREEN}${BOLD}Created CLI tool project: ${projectName}${RESET}\n`);
  console.log(`  ${DIM}cd ${projectName}${RESET}`);
  console.log(`  ${DIM}quill run cli.quill world --verbose${RESET}`);
  console.log(`  ${DIM}quill build cli.quill --standalone${RESET}  ${DIM}# Create executable${RESET}\n`);
}

// ---- Scaffold: Static Site ----
function cmdSite() {
  const projectName = process.argv[3] || 'my-site';
  ...
  fs.mkdirSync(path.join(projectName, 'pages'), { recursive: true });
  fs.mkdirSync(path.join(projectName, 'public'), { recursive: true });

  fs.writeFileSync(path.join(projectName, 'package.json'), JSON.stringify({
    name: projectName,
    version: '1.0.0',
    description: 'A static site built with Quill',
    scripts: { build: 'node build.js', dev: 'node build.js && npx serve dist' }
  }, null, 2) + '\n');

  fs.writeFileSync(path.join(projectName, 'layout.html'), `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
`);
  ...
  fs.writeFileSync(path.join(projectName, 'pages', 'index.quill'), `-- Home
component HomePage:
  to render:
    div:
...
`);
  ...
  fs.writeFileSync(path.join(projectName, 'pages', 'about.quill'), `-- About
component AboutPage:
  to render:
    div:
      h1: "About"
...
`);
  ...
  fs.writeFileSync(path.join(projectName, 'public', 'style.css'), `body {
...
`);
  ...
  console.log(`\n${GREEN}${BOLD}Created static site project: ${projectName}${RESET}\n`);
  console.log(`  ${DIM}cd ${projectName}${RESET}`);
  console.log(`  ${DIM}npm run build${RESET}        ${DIM}# Build to dist/${RESET}`);
  console.log(`  ${DIM}npx serve dist${RESET}       ${DIM}# Preview locally${RESET}\n`);
}
...
${BOLD}Scaffolding:${RESET}
  ...
  quill cli [name]             Scaffold a CLI tool project
  quill site [name]            Scaffold a static site project
