...
// Kept in sync with stdlib/runtime.go
...
const pick = (obj, ...ks) => ks.flat().reduce((o, k) => { if (k in obj) o[k] = obj[k]; return o; }, {});
const omit = (obj, ...ks) => { const keys = ks.flat(); return Object.fromEntries(Object.entries(obj).filter(([k]) => !keys.includes(k))); };
...
const writeLines = (p, lns) => require('fs').writeFileSync(p, lns.join('\n'));
...
// Drawing & Charts (Node.js: buffers commands, opens HTML in browser on exit)
const __drawCmds = [];
...
module.exports = {
  say, length, join, split,
  round, floor, ceil, abs, random, randomInt, smallest, largest, clamp, lerp, mapRange,
  formatNumber, toFixed, percent, currency, ordinal,
  toNumber, toText, typeOf,
  isText, isNumber, isList, isObject, isNothing, isFunction,
  trim, upper, lower, replace_text, startsWith, endsWith,
  padStart, padEnd, repeat, contains, capitalize, words, lines, truncate,
  range, sort, sortBy, reverse, unique, sum,
  filter, map_list, find, every, some, reduce, flat,
  includes, indexOf, slice, push, pop, concat, zip, countWhere, groupBy,
  keys, values, entries, fromEntries, merge, pick, omit, hasKey, deepCopy,
  parseJSON, toJSON,
  now, today, timestamp, formatDate, formatDateUTC, addDays, diffDays,
  wait, delay, parallel, race,
  fetchURL, fetchJSON, postJSON, putJSON, patchJSON, deleteJSON,
  regex, matches, matchGroups, matchFirst, matchesPattern, replacePattern,
  read, write, append_file, exists, readJSON, writeJSON, readLines, writeLines,
  fileExists, listFiles, deleteFile, copyFile, moveFile, makeDir, fileInfo,
  currentDir, homePath, joinPath, fileName, fileExtension, parentDir,
  env, setEnv, args, run, runAsync, platform, exit, prompt,
  hash, uuid,
  encodeBase64, decodeBase64, encodeURL, decodeURL,
  template, createServer,
  drawCircle, drawRect, drawLine, drawText, fillBackground, chart, showCanvas,
  openDB, query, execute, closeDB,
};
