...
func (g *Generator) Generate(program *ast.Program) string {
	...
	needsEnvLoader := strings.Contains(userCodeStr, "process.env.") || g.hasCommandStatements(program) || strings.Contains(userCodeStr, "Discord") || g.needsAIRuntime
...
func (g *Generator) genExpr(expr ast.Expression) string {
	...
	case *ast.AskExpression:
		g.needsAIRuntime = true
		var out strings.Builder
		// Generate inline await expression for ask
		model := `"claude-sonnet-4-20250514"`
		maxTokens := "1024"
		...
		if e.IsMessages {
			...
			out.WriteString(fmt.Sprintf(", messages: %s", g.genExpr(e.Prompt)))
			...
			out.WriteString(fmt.Sprintf(", messages: [{ role: \"user\", content: %s }]", g.genExpr(e.Prompt)))
			out.WriteString(" }); return __ai_resp.content[0].text; })())")
		}
		return out.String()
