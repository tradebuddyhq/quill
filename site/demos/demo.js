...
let nameInput = select("#demo-name");
let greetBtn = select("#demo-greet-btn");
let greetOutput = select("#demo-greet-output");
function greetUser() {
...
function checkGreetEnter(key, event) {
  if ((key === "Enter")) {
    greetUser();
  }
}
...
let count = 0;
let countDisplay = select("#demo-count");
let addBtn = select("#demo-add");
let subBtn = select("#demo-sub");
let resetBtn = select("#demo-reset");
...
let todoInput = select("#demo-todo-input");
let todoBtn = select("#demo-todo-btn");
let todoList = select("#demo-todo-list");
let todoCount = select("#demo-todo-count");
let todos = [];
function addTodo() {
  let text = getValue(todoInput);
  if ((length(text) > 0)) {
    push(todos, text);
    renderTodos();
    setValue(todoInput, "");
...
function renderTodos() {
  ...
  let items = selectAll("#demo-todo-list li");
...
function checkTodoEnter(key, event) {
  if ((key === "Enter")) {
    addTodo();
  }
}
onKeyPress(todoInput, checkTodoEnter);
let colorBtns = selectAll(".demo-color-btn");
let colorBox = select("#demo-color-box");
let colorLabel = select("#demo-color-label");
function changeColor(event) {
  let color = getAttribute(event.target, "data-color");
